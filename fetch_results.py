...
URBA_TORNEOS = [
    ...
    {"id": "2025185", "nombre": "Pre-Intermedia", "json_file": "data/urba-preinter-a.json",   "firebase_key": "urbaPreIntermedia"},
...
def fetch_urba_torneo(torneo):
    ...
    try:
        ...
        for t in teams_raw:
            name = t["name"].strip()
            team_stats[name] = {"name": name, "pts":0, "pj":0, "g":0, "e":0, "p":0,
                                "pf":0, "pc":0, "bp":0, "bf":0, "form":[]}
        for rnd in rounds:
            for match in rnd.get("matches", []):
                ...
                if match.get("local_team_offensive_bonus"):  h["pts"] += 1; h["bp"] += 1
                if match.get("visit_team_offensive_bonus"):  a["pts"] += 1; a["bp"] += 1
                if match.get("local_team_defensive_bonus"):  h["pts"] += 1; h["bf"] += 1
                if match.get("visit_team_defensive_bonus"):  a["pts"] += 1; a["bf"] += 1
